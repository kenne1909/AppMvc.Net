...
namespace App.Areas.Identity.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<IActionResult> Index(ManageMessageId? message = null)
        {
            ...
            if (user == null)
            {
                // Xử lý khi không có người dùng, ví dụ: chuyển hướng đến trang đăng nhập hoặc hiển thị thông báo lỗi
            ...
            var model = new IndexViewModel
            {
                ...
                profile = new EditExtraProfileModel()
                {
                    ...
                    HomeAdress = user.HomeAdress ?? string.Empty,
        ...
        public async Task<IActionResult> RemoveLogin(RemoveLoginViewModel account)
        {
            ...
            if (user != null)
            {
                var result = await _userManager.RemoveLoginAsync(user, account.LoginProvider ??string.Empty, account.ProviderKey ??string.Empty);
                if (result.Succeeded)
                {
                    await _signInManager.SignInAsync(user, isPersistent: false);
        ...
        public async Task<IActionResult> AddPhoneNumber(AddPhoneNumberViewModel model)
        {
            ...
            if (user == null)
            {
                TempData["ErrorMessage"] = "Người dùng không được xác thực. Vui lòng đăng nhập lại.";
                // Xử lý khi không có người dùng, ví dụ: chuyển hướng đến trang đăng nhập hoặc hiển thị thông báo lỗi
                return RedirectToAction("Login", "Account");
            }
            var code = await _userManager.GenerateChangePhoneNumberTokenAsync(user, model.PhoneNumber??string.Empty);
            await _emailSender.SendSmsAsync(model.PhoneNumber ??string.Empty, "Mã xác thực là: " + code);
        ...
        public async Task<IActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            if (user != null)
            {
                var result = await _userManager.ChangePhoneNumberAsync(user, model.PhoneNumber ??string.Empty, model.Code ??string.Empty);
                if (result.Succeeded)
                {
                    await _signInManager.SignInAsync(user, isPersistent: false);
        ...
        public async Task<IActionResult> GenerateRecoveryCode()
        {
            ...
            if (user != null)
            {
                ...
                // Kiểm tra nếu codes là null
                if (codes == null)
                {
                    _logger.LogWarning("Failed to generate new recovery codes.");
