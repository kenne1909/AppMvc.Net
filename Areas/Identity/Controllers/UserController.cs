...
namespace App.Areas.Identity.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> AddRoleAsync(string id)
        {
            ...
            List<string> roleNames = await _roleManager.Roles.Select(r => r.Name ?? string.Empty).ToListAsync();
        ...
        public async Task<IActionResult> AddRoleAsync(string id, [Bind("RoleNames")] AddUserRoleModel model)
        {
            ...
            var roleName = model.RoleNames ?? Array.Empty<string>();
        ...
        public async Task<IActionResult> SetPasswordAsync(string id, SetUserPasswordModel model)
        {
            ...
            {
                // Xử lý khi mật khẩu mới là null hoặc trống, ví dụ: trả về lỗi hoặc thông báo cho người dùng
                ModelState.AddModelError(string.Empty, "Mật khẩu mới không được để trống.");
                return View(model);
            }
        ...
        public async Task<ActionResult> AddClaimAsync(string userid, AddUserClaimModel model)
        {
            ...
            await _userManager.AddClaimAsync(user, new Claim(model.ClaimType ?? string.Empty, model.ClaimValue ?? string.Empty));
        ...
        public async Task<IActionResult> EditClaim(int claimid)
        {
            ...
            if (userclaim == null)
            {
                // Xử lý khi không tìm thấy userclaim
        ...
        private async Task GetClaims(AddUserRoleModel model)
        {
            if (model.user == null)
            {
                // Xử lý khi model.user là null
                throw new ArgumentNullException(nameof(model.user), "User không thể null.");
            }
