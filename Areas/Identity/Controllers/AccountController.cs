    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string? returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var result = await _signInManager.PasswordSignInAsync(model.UserNameOrEmail ??string.Empty, model.Password ??string.Empty, model.RememberMe, lockoutOnFailure: true);                
                // Tìm UserName theo Email, đăng nhập lại
                if ((!result.Succeeded) && AppUtilities.IsValidEmail(model.UserNameOrEmail ??string.Empty))
                {
                    var user = await _userManager.FindByEmailAsync(model.UserNameOrEmail ??string.Empty);
                    if (user != null)
                    {
                        result = await _signInManager.PasswordSignInAsync(user.UserName ??  "defaultUserName", model.Password ??string.Empty, model.RememberMe, lockoutOnFailure: true);
                    }
                } 
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string? returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (callbackUrl != null)
                    {
                        await _emailSender.SendEmailAsync(model.Email ??string.Empty, 
                    ...
                    {
                        // Xử lý trường hợp callbackUrl là null, ví dụ: báo lỗi hoặc gán một giá trị mặc định.
                    }


                    var registerConfirmationUrl = Url.Action(nameof(RegisterConfirmation));
                    if (_userManager.Options.SignIn.RequireConfirmedAccount && registerConfirmationUrl != null)
                    {
                        return LocalRedirect(registerConfirmationUrl);
                    }
                    else
                    {
        ...
        public async Task<IActionResult> ExternalLoginCallback(string? returnUrl = null, string? remoteError = null)
        {
            ...
            else
            {
                ...
                {
                    // Xử lý khi email là null, ví dụ: báo lỗi hoặc chuyển hướng đến một trang khác.
        ...
        public async Task<IActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string? returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // Input.Email
                var registeredUser = await _userManager.FindByEmailAsync(model.Email ??string.Empty);
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            var code = Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(model.Code ??string.Empty));

            var result = await _userManager.ResetPasswordAsync(user, code, model.Password ??string.Empty);
            if (result.Succeeded)
            {
                return RedirectToAction(nameof(AccountController.ResetPasswordConfirmation), "Account");
            }
        ...
        public async Task<IActionResult> SendCode(SendCodeViewModel model)
        {
            ...
            var code = await _userManager.GenerateTwoFactorTokenAsync(user, model.SelectedProvider ??string.Empty);
            ...
            else if (model.SelectedProvider == "Phone")
            {
                ...
                if (phoneNumber != null)
                {
                    await _emailSender.SendSmsAsync(phoneNumber, message);
                } 
        ...
        public async Task<IActionResult> VerifyCode(VerifyCodeViewModel model)
        {
            ...
            var result = await _signInManager.TwoFactorSignInAsync(model.Provider ??string.Empty, model.Code ??string.Empty, model.RememberMe, model.RememberBrowser);
            if (result.Succeeded)
            {
        ...
        public async Task<IActionResult> VerifyAuthenticatorCode(bool rememberMe, string? returnUrl = null)
        {
            ...
            return View(new VerifyAuthenticatorCodeViewModel { ReturnUrl = returnUrl ?? string.Empty, RememberMe = rememberMe });
        }
        ...
        public async Task<IActionResult> VerifyAuthenticatorCode(VerifyAuthenticatorCodeViewModel model)
        {
            ...
            // If a user enters incorrect codes for a specified amount of time then the user account
            // will be locked out for a specified amount of time.
            var result = await _signInManager.TwoFactorAuthenticatorSignInAsync(model.Code ??string.Empty, model.RememberMe, model.RememberBrowser);
            if (result.Succeeded)
            {
                return LocalRedirect(model.ReturnUrl);
            }
        ...
        public async Task<IActionResult> UseRecoveryCode(string? returnUrl = null)
        {
            ...
            return View(new UseRecoveryCodeViewModel { ReturnUrl = returnUrl ?? string.Empty });
        }
        ...
        public async Task<IActionResult> UseRecoveryCode(UseRecoveryCodeViewModel model)
        {
            ...
            var result = await _signInManager.TwoFactorRecoveryCodeSignInAsync(model.Code ??string.Empty);
            if (result.Succeeded)
            {
                return LocalRedirect(model.ReturnUrl);
            }
